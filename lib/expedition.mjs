...
export function makeExpedition(rng=Math.random){
 ...
 const ocean=animals.filter(a=>!['dog','cow','cat'].includes(a.id));
 const familiarTargets=shuffle([animals.find(a=>a.id==='dog'),animals.find(a=>a.id==='cow')],rng);
 const oceanTarget=shuffle(ocean,rng)[0];
 const targets=[...familiarTargets,oceanTarget];
 return targets.map((target,index)=>{
  let choices;
  if(index<2){
   choices=shuffle(familiar,rng);
  }else{
   const decoys=shuffle(ocean.filter(a=>a.id!==target.id),rng).slice(0,2);
   choices=shuffle([target,...decoys],rng);
  }
  const duration=index<2?Math.min(2.8,target.id==='cat'?1.2:2.8):4;
  return {target:target.id,present:true,duration,choices,layers:[{id:target.id,start:0,duration,offsetFraction:index<2?0:rng()*.75,gain:index<2?1.25:1.15,pan:0,target:true}],style:index};
 });
}
