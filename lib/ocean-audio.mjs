...
export class OceanAudio {
 ...
 async load(base,onProgress=()=>{}){
  ...
  this.loading=Promise.all(animals.map(async({id,audioUrl})=>{
   if(this.buffers.has(id))return;
   const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),20000);
   try {
    const source=audioUrl||`${base}audio/${id}.mp3`;
    const r=await fetch(source,{signal:controller.signal,mode:'cors'});if(!r.ok)throw Error('Recording unavailable');
 ...
 play(layers,duration,onComplete=()=>{},onError=()=>{}){
  ...
  try{
   for(const layer of layers){
    ...
    const offset=Math.min(layer.offsetFraction!=null?layer.offsetFraction*Math.max(0,buffer.duration-layer.duration):(layer.offset||0),Math.max(0,buffer.duration-layer.duration));
