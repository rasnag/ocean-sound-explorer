...
export class OceanAudio {
 ...
 async load(base,onProgress=()=>{}){
  ...
  this.loading=Promise.all(animals.map(async animal=>{
   if(this.buffers.has(animal.id))return;
   const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),20000);
   try {
    const src=animal.source||`${base}audio/${animal.id}.mp3`;
    const r=await fetch(src,{signal:controller.signal,mode:'cors'});if(!r.ok)throw Error('Recording unavailable');
    ...
    const scale=Math.min(.16/Math.max(Math.sqrt(energy/count),.0001),.85/Math.max(peak,.0001));
    for(let c=0;c<buffer.numberOfChannels;c++){const d=buffer.getChannelData(c);for(let i=0;i<d.length;i++)d[i]*=scale;}
    this.buffers.set(animal.id,buffer);onProgress(this.buffers.size);
 ...
 prepare(){
  ...
  for(const animal of animals){
   const duration=Math.min(5,this.buffers.get(animal.id).duration);
   this.prepareClip([{id:animal.id,start:0,duration,offset:0,gain:.9,pan:0}],duration);
  }
 }
 prepareClip(layers,duration){
  ...
  for(const layer of layers){const b=this.buffers.get(layer.id);const offset=Math.min(layer.offsetFraction!=null?layer.offsetFraction*Math.max(0,b.duration-layer.duration):(layer.offset||0),Math.max(0,b.duration-layer.duration));const length=Math.min(layer.duration,b.duration-offset);const start=Math.round(layer.start*rate);const count=Math.floor(length*rate);const l=b.getChannelData(0),r=b.getChannelData(Math.min(1,b.numberOfChannels-1));const angle=((layer.pan||0)+1)*Math.PI/4;
 ...
 playAnimal(id,onComplete=()=>{},onError=()=>{}){
  const buffer=this.buffers.get(id);if(!buffer){onError(Error('Please load the recordings first.'));return null;}
  const duration=Math.min(5,buffer.duration);return this.play([{id,start:0,duration,offset:0,gain:.9,pan:0}],duration,onComplete,onError);
 }
 play(layers,duration,onComplete=()=>{},onError=()=>{}){
  this.stop();const token=this.generation;const clip=this.clips.get(JSON.stringify([layers,duration]));
  if(clip&&this.player){
   try{if(globalThis.navigator?.audioSession)globalThis.navigator.audioSession.type='playback';}catch{}
   const player=this.player;player.src=clip;player.currentTime=0;player.volume=1;player.muted=false;this.active=true;
   player.onended=()=>{if(token===this.generation){this.stop();onComplete();}};player.onerror=()=>{if(token===this.generation){this.stop();onError(Error('Media playback failed'));}};
  ...
  const resume=this.unlock();const c=this.context;const origin=c.currentTime+.04;this.active=true;
  try{for(const layer of layers){const buffer=this.buffers.get(layer.id);if(!buffer)throw Error('Please load the recordings first.');const source=c.createBufferSource(),gain=c.createGain();source.buffer=buffer;const offset=Math.min(layer.offset||0,Math.max(0,buffer.duration-layer.duration));const length=Math.min(layer.duration,buffer.duration-offset);const start=origin+layer.start;gain.gain.setValueAtTime(0,start);gain.gain.linearRampToValueAtTime(layer.gain,start+.08);gain.gain.setValueAtTime(layer.gain,start+Math.max(.09,length-.18));gain.gain.linearRampToValueAtTime(0,start+length);source.connect(gain);gain.connect(c.destination);source.start(start,offset,length);this.nodes.push(source,gain);}const end=c.createBufferSource();end.buffer=c.createBuffer(1,1,c.sampleRate);end.connect(c.destination);end.onended=()=>{if(this.generation===token&&this.active){this.stop();onComplete();}};end.start(origin+duration);this.nodes.push(end);}catch(e){this.stop();onError(e);return null;}
  Promise.resolve(resume).catch(e=>{if(token===this.generation){this.stop();onError(e);}});return {origin,duration,context:c};
 }
