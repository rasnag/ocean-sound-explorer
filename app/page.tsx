...
export default function Home(){
 ...
 return <main className="expedition">
  ...
  {screen==='intro'?<section className="console intro"><div className="intro-photo"><img src="./animals/dolphin.webp" alt="A dolphin"/><span className="mission-tag"><Headphones size={18}/> SOUND MISSION</span></div><div className="intro-body"><span className="eyebrow">AHOY, EXPLORER!</span><h1>Who made<br/>that sound?</h1><p className="story">Listen to a mystery ocean sound. Then tap the animal you think made it.</p><button className="primary big" onClick={start}><Play fill="currentColor"/> Start!</button><span className="small">3 quick sounds · Real ocean animals</span></div></section>:
  screen==='done'?<section className="console done"><span className="medal">🌊</span><span className="eyebrow">MISSION COMPLETE</span><h1>Great listening!</h1><p>You discovered {ROUND_COUNT} ocean sounds.</p><p className="score">★ {score} / {ROUND_COUNT}</p><button className="primary big" onClick={start}><RotateCcw/> Play again!</button></section>:
  ...
  <section className="console game"><div className="control-panel" style={{width:'100%',maxWidth:760,margin:'0 auto'}}>
   {!ready?<div className="loading" role="status"><Headphones size={36}/><h2>Getting your mystery sound…</h2><Progress value={loaded*20}/><p>{loaded} of 5 sounds ready</p>{error&&<><p>{error}</p><button className="primary big" onClick={load}>Try again</button></>}</div>:<>
    <div style={{textAlign:'center'}}><span className="eyebrow">MYSTERY SOUND {index+1}</span><h1>👂 Listen!</h1><h2>Who made this sound?</h2></div>
    ...
    {heard&&<><p className="play-status">Now tap an animal!</p><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>{plan.choices.map((a:any)=><button key={a.id} onClick={()=>choose(a.id)} disabled={!!answer} style={{padding:10,borderRadius:20,border:answer===a.id?'4px solid currentColor':'2px solid #d8e8ef',background:'white',fontSize:'1rem',fontWeight:800,cursor:'pointer'}}><img src={'./animals/'+a.id+'.webp'} alt={a.name} style={{width:'100%',aspectRatio:'1/1',objectFit:'cover',borderRadius:14}}/><span style={{display:'block',marginTop:8}}>{a.name}</span></button>)}</div><button className="mix-button big" onClick={play} disabled={playing}><Volume2/> Hear it again</button></>}
    {answer&&<div className="feedback" role="status"><div className="feedback-title"><span>{answer===plan.target?'🌟':'🐋'}</span><h2>{answer===plan.target?'You found it!':`It was the ${target?.name}!`}</h2></div><p className="fact">{target?.fact}</p><button className="primary big" onClick={next}>{index===ROUND_COUNT-1?'Finish!':'Next sound →'}</button></div>}
   </>}
  </div></section></>}
